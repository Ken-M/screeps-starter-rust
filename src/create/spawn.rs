...
/// 生産する。最小個体を刻んで出しても討たれて溶けるだけなので、厚い1体に賭ける。
///
/// 値は spawn 単体の容量 (300)。経済壊滅時は extension へ補給する hauler が
/// いないため、部屋の容量 (800) ではなくここが実際の上限になる。
/// これを超える額を目標にすると永久に到達せず、敵が去るまで一切生産しない
/// 死に設定になる (当初 550 と置いて 139/550 で頭打ちになった)。
const SIEGE_SAVE_TARGET: u32 = 300;
